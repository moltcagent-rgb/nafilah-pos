...
export async function createMenuItem({ name, price, category, is_available }) {
  const id = randomUUID();
  const timestamp = nowIso();
  const result = await getClient().execute({
    sql: `INSERT INTO menu_items (id, name, price, category, is_available, created_at, updated_at)
          VALUES (?, ?, ?, ?, ?, ?, ?) RETURNING *`,
    args: [id, name, price, category, is_available ? 1 : 0, timestamp, timestamp],
  });
  return rowToMenuItem(result.rows[0]);
}

export async function updateMenuItem(id, fields) {
  const allowed = ['name', 'price', 'category', 'is_available'];
  const sets = [];
  const args = [];
